-- ============================================================================
-- WMS Configuration Schema - ENHANCED VERSION
-- Compatible cu structura existentă de locations
-- ============================================================================
...
-- ============================================================================
-- 2. LOCATIONS - EXTEND EXISTING TABLE
-- ============================================================================
...
-- ALTER existing locations table to add new columns
-- Note: locations table already exists with id VARCHAR(50)
DO $$
BEGIN
    -- Add warehouse_id if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='warehouse_id') THEN
        ALTER TABLE locations ADD COLUMN warehouse_id UUID REFERENCES warehouses(id) ON DELETE CASCADE;
    END IF;
    
    -- Add zone_id if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='zone_id') THEN
        ALTER TABLE locations ADD COLUMN zone_id UUID REFERENCES warehouse_zones(id) ON DELETE CASCADE;
    END IF;
    
    -- Add location_type_id if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='location_type_id') THEN
        ALTER TABLE locations ADD COLUMN location_type_id UUID REFERENCES location_types(id);
    END IF;
    
    -- Add location_code (unique identifier) if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='location_code') THEN
        ALTER TABLE locations ADD COLUMN location_code VARCHAR(100) UNIQUE;
    END IF;
    
    -- Add barcode if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='barcode') THEN
        ALTER TABLE locations ADD COLUMN barcode VARCHAR(200) UNIQUE;
    END IF;
    
    -- Add qr_code if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='qr_code') THEN
        ALTER TABLE locations ADD COLUMN qr_code TEXT;
    END IF;
    
    -- Rename existing columns to new convention
    -- aisle already exists (rack -> aisle in old schema, keeping both)
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='aisle') THEN
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='rack') THEN
            -- Don't rename, keep rack, add aisle separately
            ALTER TABLE locations ADD COLUMN aisle VARCHAR(20);
        END IF;
    END IF;
    
    -- Add shelf_level if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='shelf_level') THEN
        ALTER TABLE locations ADD COLUMN shelf_level INTEGER;
    END IF;
    
    -- Add bin_position if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='bin_position') THEN
        ALTER TABLE locations ADD COLUMN bin_position VARCHAR(10);
    END IF;
    
    -- Add dimensions if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='width_cm') THEN
        ALTER TABLE locations ADD COLUMN width_cm DECIMAL(10,2);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='depth_cm') THEN
        ALTER TABLE locations ADD COLUMN depth_cm DECIMAL(10,2);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='height_cm') THEN
        ALTER TABLE locations ADD COLUMN height_cm DECIMAL(10,2);
    END IF;
    
    -- Add capacity columns if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='max_pallets') THEN
        ALTER TABLE locations ADD COLUMN max_pallets INTEGER DEFAULT 1;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='max_weight_kg') THEN
        ALTER TABLE locations ADD COLUMN max_weight_kg DECIMAL(10,2);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='max_volume_cubic_meters') THEN
        ALTER TABLE locations ADD COLUMN max_volume_cubic_meters DECIMAL(10,2);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='current_occupancy_percent') THEN
        ALTER TABLE locations ADD COLUMN current_occupancy_percent DECIMAL(5,2) DEFAULT 0;
    END IF;
    
    -- Add status if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='status') THEN
        ALTER TABLE locations ADD COLUMN status VARCHAR(50) DEFAULT 'AVAILABLE';
    END IF;
    
    -- Add is_pickable if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='is_pickable') THEN
        ALTER TABLE locations ADD COLUMN is_pickable BOOLEAN DEFAULT true;
    END IF;
    
    -- Add priority if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='priority') THEN
        ALTER TABLE locations ADD COLUMN priority INTEGER DEFAULT 5;
    END IF;
    
    -- Add physical properties if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='temperature_controlled') THEN
        ALTER TABLE locations ADD COLUMN temperature_controlled BOOLEAN DEFAULT false;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='requires_forklift') THEN
        ALTER TABLE locations ADD COLUMN requires_forklift BOOLEAN DEFAULT false;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='accessibility') THEN
        ALTER TABLE locations ADD COLUMN accessibility VARCHAR(50) DEFAULT 'MEDIUM';
    END IF;
    
    -- Add tracking columns if not exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='last_inventory_check') THEN
        ALTER TABLE locations ADD COLUMN last_inventory_check TIMESTAMP;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='last_modified_at') THEN
        ALTER TABLE locations ADD COLUMN last_modified_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='locations' AND column_name='created_by') THEN
...
CREATE TABLE IF NOT EXISTS product_location_preferences (
    ...
    location_id VARCHAR(50) NOT NULL REFERENCES locations(id) ON DELETE CASCADE,  -- VARCHAR to match existing locations.id
...
CREATE TABLE IF NOT EXISTS package_instances (
    ...
    -- Tracking
    current_location_id VARCHAR(50) REFERENCES locations(id),  -- VARCHAR to match existing locations.id
    current_batch_id UUID REFERENCES product_batches(id),
    
    -- Rental tracking (for returnable packages)
    rented_to_customer_id UUID,  -- Will be linked when customers table exists
    rented_at TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS batch_state_history (
    ...
    -- Context
    from_location_id VARCHAR(50) REFERENCES locations(id),  -- VARCHAR to match existing locations.id
    to_location_id VARCHAR(50) REFERENCES locations(id),    -- VARCHAR to match existing locations.id
...
CREATE INDEX IF NOT EXISTS idx_zones_type ON warehouse_zones(zone_type);
CREATE INDEX IF NOT EXISTS idx_zones_active ON warehouse_zones(is_active);

-- Locations (new columns only)
...
CREATE INDEX IF NOT EXISTS idx_locations_is_pickable ON locations(is_pickable);
...
CREATE INDEX IF NOT EXISTS idx_packaging_types_active ON packaging_types(is_active);
CREATE INDEX IF NOT EXISTS idx_package_instances_code ON package_instances(instance_code);
CREATE INDEX IF NOT EXISTS idx_package_instances_status ON package_instances(status);

-- Carriers
CREATE INDEX IF NOT EXISTS idx_carriers_code ON shipping_carriers(carrier_code);
CREATE INDEX IF NOT EXISTS idx_carriers_active ON shipping_carriers(is_active);
CREATE INDEX IF NOT EXISTS idx_carriers_preferred ON shipping_carriers(is_preferred);

-- Vehicles
CREATE INDEX IF NOT EXISTS idx_vehicles_code ON internal_vehicles(vehicle_code);
CREATE INDEX IF NOT EXISTS idx_vehicles_warehouse ON internal_vehicles(warehouse_id);
CREATE INDEX IF NOT EXISTS idx_vehicles_status ON internal_vehicles(current_status);
CREATE INDEX IF NOT EXISTS idx_vehicles_driver ON internal_vehicles(assigned_driver_id);
...
-- Workflow
CREATE INDEX IF NOT EXISTS idx_workflow_states_warehouse ON workflow_states(warehouse_id);
CREATE INDEX IF NOT EXISTS idx_workflow_states_code ON workflow_states(state_code);
CREATE INDEX IF NOT EXISTS idx_workflow_transitions_warehouse ON workflow_transitions(warehouse_id);
CREATE INDEX IF NOT EXISTS idx_workflow_transitions_from ON workflow_transitions(from_state_id);
CREATE INDEX IF NOT EXISTS idx_workflow_transitions_to ON workflow_transitions(to_state_id);
CREATE INDEX IF NOT EXISTS idx_batch_state_history_batch ON batch_state_history(batch_id);
CREATE INDEX IF NOT EXISTS idx_batch_state_history_state ON batch_state_history(to_state_id);

-- Settings
CREATE INDEX IF NOT EXISTS idx_warehouse_settings_warehouse ON warehouse_settings(warehouse_id);
CREATE INDEX IF NOT EXISTS idx_warehouse_settings_category ON warehouse_settings(setting_category);
...
-- Apply trigger to NEW tables only (locations already has trigger)
...
ON CONFLICT (warehouse_id, state_code) DO NOTHING;
...
COMMENT ON TABLE locations IS 'Physical storage locations (racks, shelves, bins) - EXTENDED from existing schema';
